//#############################################################################
// FILE:   HW4CJS_Project_main.c
//
// TITLE:  HW4 Personal Project Code.
// Objective: Control a light switch based on whistle tones.
//#############################################################################
...
__interrupt void ADCB_ISR(void);    //CJS ADCB Interrupt PreDefinition.
...
//Global Variables for Servo A control
...
//    EPwm9Regs.CMPA.bit.CMPA = 0;       //CJS - Disabled. State of PWM based on TBPRD.
    ...
    // ---------------- SERVO 1 ON GPIO14 VIA EPWM8A Setup ----------------
    EALLOW;
    ...
    //CJS ADCB Initialization and Register configuration
    AdcbRegs.ADCCTL2.bit.PRESCALE = 6; //set ADCCLK divider to /4
    AdcSetMode(ADC_ADCB, ADC_RESOLUTION_12BIT, ADC_SIGNALMODE_SINGLE); //read calibration settings
    AdcbRegs.ADCCTL1.bit.INTPULSEPOS = 1;   //CJS Set pulse positions to late.
    AdcbRegs.ADCCTL1.bit.ADCPWDNZ = 1;  //CJS Power up ADCB
    ...
    //CJS Enable ADCB1 Interrupt 1.2
    ...
    while(1)
    {
        ...
        if ( (pingFFT == 1) || (pongFFT == 1) ) {
            ...
            micFrequency = maxpwrindex*10000.0/1024.0;
            UARTPrint = 1;
        }
        //!!!!!!!!!!!!!!!!!!!!!!  End of Block
    }
